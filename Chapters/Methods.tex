\chapter{Methods}
    ...
    The process by which the XRD is used in this work to probe increasingly deeper depths is called grazing incidence x-ray diffraction\cite{pandey_structural_2021} (GIXRD; also called glancing incidence x-ray diffraction\cite{COLOMBI2007554}).
    ...
    This is due to the exponential attenuation of intensity as the beam travels; given by the equation: 
    \begin{equation}
        I(y)=I_0e^{-\alpha y}
        \label{eq: attenuation}
    \end{equation}
    % $$I(y)=I_0e^{-\alpha y}$$
    with $\alpha=2\omega n_I/c$\cite{https://doi.org/10.1002/jps.22202,hecht}. 
    With the complex index of refraction ($n_I$) strongly dependent on the incoming wavelength. 
    The x-ray source is copper, producing $K_\alpha$ in abundance with some $K_\beta$. 
    Since one wavelength provides the most uniformity on the diffractogram, the incident beam optics include a filter to limit the unnecessary wavelengths. 
    $K_\alpha$ is what is most common, so the $K_\beta$ gets mostly filtered out by a nickel filter. 
    This results in a x-ray beam that has a wavelength of 1.54\AA.
    It is these subtle differences that allow for the determination of different ratios of dopant to substrate.

    % Picture of GIXRD schematic
    \begin{figure}[H]
        \centering
        % \begin{tikzpicture}[>=latex] % Use latex arrow heads
    ...
    The methods are: 1. Intensity\cite{pandey_structural_2021}, 2. Curve Fitting, and 3. Summation.
    Before discussing the methods used to analyze the sample, it is important to know what assumptions were made. 
    % Before discussing the intricacies of these methods, it is important to first know the shared simplifying assumptions on which they are predicated for this study.
    ...
    The next assumption is from the exponential nature of attenuation; 
    it is that the intensity of the x-ray has been attenuated by 90\%. 
    Then thirdly, that the sample is put in its preferred orientation that allows for the silicon structure to be aligned with the x-ray beam.
    ...
    \subsubsection*{Method 1: Intensity}
        ...
        This method takes the highest intensity count from the amorphous hump, and the highest intensity count from the crystalline peaks as input parameters, and calculates the ratio of them.
        ...
        \begin{equation}
            m=\frac{I_a}{I_a+I_c}  
            \label{eq: mixture equation}
        \end{equation}
        % $$m=\frac{I_a}{I_a+I_c}$$
        Where $I_a$ is the highest count of the amorphous hump, $I_c$ is the highest count of the crystalline peaks, and $m$ is the mixture percent.
    ...
    \subsubsection*{Method 2: Curve Fitting}
        ...
        \begin{figure}[H]
            \centering
            \includesvg[width=\linewidth]{Pictures/All_XRD_Fits.svg}
            \caption{Diffractogram with relevant peaks fitted, and incident angle shown}
            \label{fig: fitted peaks}
        \end{figure}

        The amorphous hump, and the crystalline peaks can be approximated by Gaussian curves.
        As such, it is possible to fit a curve to the hump, and the desired peak, as well as a background baseline. 
        ...
        This method also allows for the background baseline to be subtracted out.
    ...
    \subsubsection*{Method 3: Summation}
        ...
        This method is similar to the curve fitting method, but instead of fitting the hump and peak to a Gaussian, a numerical integration is performed directly on the data.
        This was implemented to compare to the curve fit method due to mismatches in some of the fits and their peaks (see figure \ref{fig: fitted peaks}). 
        ...
        The mixture percentage is derived as follows:
        ...
        \end{equation}
        % $$m=\frac{\sum_{\theta_i}^{\theta_f}y_a}{\sum_{\theta_i}^{\theta_f}y_a+\sum_{\theta_i}^{\theta_f}y_c}$$
        Where $y_a$, and $y_c$ are the counts of the current $\theta$ position for the amorphous and crystalline parts respectively.
        With $dx_a$, and $dx_c$ as the spacing between the $\theta$ positions for the amorphous and crystalline parts respectively.
        The quantity $m$ is the mixture amount, and $\theta_f$, and $\theta_i$ are the starting and ending $\theta$ positions for the amorphous hump or crystalline peak respectively. 
        This method was developed as a comparison to the curve fit method as some fits are not ideal (as figure \ref{fig: fitted peaks} shows). 
