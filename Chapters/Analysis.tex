\chapter{Analysis} % Potentially talk about uncertainty here or in the methods chapter
    ...
    The $\omega=0.005^\circ$ line shows that the depth is composed of a value greater than unity for the sample; This is physically impossible.
    The greater than unity error comes about from having no crystalline peak from the silicon, so the amorphous hump from the phosphorous is the only characteristic present on the diffractogram (this is the diffractogram in figure \ref{fig:pure amorphous XRD scan}).
    ...
    The agreement of those methods show that only one needs to be computed for future analysis.
